# Code Before DSA: The Ultimate Logic WarmUp Archive before you dive into DSA!

> 🚀 **Strengthen your logic, problem-solving, and programming fundamentals before diving into DATA STRUCTURE & ALGORITHMS!**

Welcome to **Code Before DSA** - a comprehensive YouTube series where I, **[Shubhashish Chakraborty](https://imshubh.site)**, solve 100–150 beginner/intermediate-level coding and logic-building problems. Each problem comes with a clear explanation, multiple language solutions *(intially on C++ only!)*, and an accompanying YouTube video to help you build a solid foundation.

## 📺 Watch the Series

### Watch the complete series here on **[ShubhDEVs](https://youtube.com/@shubhdevs)** → **[YouTube Playlist](https://youtube.com/playlist?list=PLsWLOBFxUwM53Kr1RUqraEQbUcT5Jq3Jk)**

## 🎯 About This Repository

This repository is designed to help beginners:

- ✅ **Master Core Programming Fundamentals**: Build strong logic and problem-solving skills
- ✅ **Practice Before DSA**: Strengthen conditionals, loops, arrays, and basic algorithms, and many more...
- ✅ **Learn Multiple Languages**: Solutions provided in C++, Python, Java, and more..
- ✅ **Follow Along with Videos**: Each problem has a detailed YouTube explanation..!


## 🚀 How to Use

1. **Browse Problems** 📚
   - Navigate to the **[problems](./problems/README.md)** directory
   - Read through the problem list in `problems/README.md`
   - Choose a problem that interests you

2. **Understand the Problem** 💭
   - Read the problem description carefully
   - Check the examples and expected outputs
   - Think about the approach before coding

3. **Try Solving It Yourself** ✏️
   - Attempt to solve the problem on your own
   - Write your solution in your preferred language
   - Test with the given examples

4. **Check Solutions** 🔍
   - Review the provided solutions in `/solutions`
   - Compare your approach with the given solutions
   - Learn different ways to solve the same problem

5. **Watch the Video** 🎥
   - Click the YouTube video link for the problem
   - Follow along with the step-by-step explanation
   - Understand the thought process and optimization tips

## 🤝 Contributing

Contributions are welcome and greatly appreciated! ✨

There are many ways you can contribute to this project:

- 💻 **Add New Solutions** — Implement solutions in languages not yet covered
- 🐛 **Fix Bugs** — Report or fix any errors in existing solutions
- 📝 **Improve Documentation** — Enhance problem descriptions or explanations
- ✨ **Suggest Improvements** — Propose better algorithms or code optimizations

### How to Contribute

1. **Fork this repository** 🍴
2. **Create a new branch** (`git checkout -b feature/your-feature-name`)
3. **Make your changes** (add solutions, fix typos, improve docs)
4. **Commit your changes** (`git commit -m 'Add: Description of your contribution'`)
5. **Push to your branch** (`git push origin feature/your-feature-name`)
6. **Submit a Pull Request** 🚀

Please ensure your code follows the existing style and includes comments where necessary.

## 📚 Topics Covered

- Basic Programming & Logic Building
- Conditionals & Loops
- Arrays & String Manipulation
- Number Theory & Mathematical Computations
- Pattern Printing
- Recursion Basics
- Sorting & Searching Fundamentals

## 💡 Credits

Built with ❤️ by [Shubhashish Chakraborty](https://github.com/Shubhashish-Chakraborty)

Inspired by [Prince Singh CSE](https://www.youtube.com/@PrinceSinghCSE)

---

⭐ **Star this repo** if you find it helpful! Your support means a lot! ⭐

